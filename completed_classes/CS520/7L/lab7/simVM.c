...
//sizes of virtural and phsyical memory and size of pages in words
int vm_size;
int pm_size;
int page_size;
...
static void *read(void *handle, unsigned int address) {
    ...
    if (page_table[page_num]->ppn >= 0) {   //entry is in physical memory
        ...
        if (TRACE) {
            printf("value at address %d in Virtual Page Number %d not in virtual memory, replacing Physical Page Number %d\n", address, page_num, rr_next_replace);
        }
        ++page_faults;
        page_table_entry* replace = p_memory[rr_next_replace]; // entry to replace    
        replace->ppn = -1;
        ...
        page_table[page_num]->ppn = rr_next_replace;
        p_memory[rr_next_replace] = page_table[page_num];
        ++rr_next_replace;
        rr_next_replace = rr_next_replace % pm_size;
        return memory[address];
    }
}
...
static void check_address(int address) {
