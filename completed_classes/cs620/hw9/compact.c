...
void defrag(){
    //move all programs to the start of memory
    pgm* tmp_pgm = _root_pgm->next;
    int offset = 0;
    while (tmp_pgm != NULL) {
        tmp_pgm->start = offset;
        offset += tmp_pgm->size;
        tmp_pgm = tmp_pgm->next;
    }

    //update first hole in list to be the freed memory at the end
    hole* big_hole = _root_hole->next;
    big_hole->start = offset;
    big_hole->size = size - offset;

    //free the rest of the holes
    hole* tmp_hole=big_hole->next;
    big_hole->next = NULL;
    while(tmp_hole != NULL) {
        hole *deall = tmp_hole;
