...
int readData(char filename[] , ipInfo_t addr[]){
    ...
    for (int j = 0; j < i; ++j) {
        ...
        for (int l = 0; l < 4; ++l){
            ...
            for (int n = 0; n < 8; ++n) {
                IPadress += (octint & 0b10000000 ? 1 : 0) * pow(2,h);
                octint = octint << 1;
                --h;
    ...
    for (int j = 0; j < i; ++j) {
        ...
        for (int l = 0; l < 4; ++l){
            ...
            for (int n = 0; n < 8; ++n) {
                ...
                subNetMask += (octint & 0b10000000 ? 1 : 0) * pow(2,h);
                octint = octint << 1;
                --h;
...
void computeValues(ipInfo_t addr[], int size){
    for (int i = 0; i < size; ++i) {
        if (((addr[i].ipAdress >> 31) & 0x1) == 0) {
            addr[i].networkClass = 'A';
            printf("\ntestclassA");
        }
        else if (((addr[i].ipAdress >> 30) & 0x1) == 0) {
            addr[i].networkClass = 'B';
            printf("\ntestclassB");
        }
        else if (((addr[i].ipAdress >> 30) & 0x1) == 1) {
            addr[i].networkClass = 'C';
            printf("\ntestclassC");
        }
        else {
            printf("\n >>ipAdress has invalid network class");
        }
...
void printResults(char filename[], ipInfo_t addr[], int size) {
    ...
    for (int i = 0; i < size; ++i) {
        fprintf(infilep, "%20s     %08x%20s     %08x%8c%8d%8d\n", addr[i].ipAdressDot, addr[i].ipAdress, addr[i].subnetMaskDot, addr[i].subnetMask, addr[i].networkClass, addr[i].totalSubnets, addr[i].totalHosts);  
    }
}
