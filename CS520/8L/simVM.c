    ...
    if (page_table[page_num]->ppn >= 0) {   //entry is in physical memory
        ...
        if (pm_replace_alg) {  // entry to replace
            ...
            for(int i = 0; i < pm_size; ++i) {
                if (tlb[i]->page_time < oldest_time) {
                    replace_index = i;
                    oldest_time = tlb[i]->tlb_time;
                }
        ...
        if (pm_replace_alg) {
            replace = page_table[page_num];
            replace->page_time = curr_time;
        } else {
